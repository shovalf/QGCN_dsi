...
import numpy as np
from QGCN_model.QGCN_ssp import QGCN
...
class QGCNActivator:
    def __init__(self, model: QGCN, params, train_data: GraphsDataset, nni=False, device="cuda:0"):
        self._nni = nni
        ...
        self._predictioner = KFAC(self._model, 
                eps=-1, 
                sua=False, 
                pi=False, 
                update_freq=8,
    ...
    def train(self, show_plot=True, early_stop=False):
        ...
        for epoch_num in range(self._epochs):
            lam = (float(epoch_num)/float(self._epochs))**3.
            if not self._nni:
            ...
            for batch_index, (A, x0, embed, label) in enumerate(self._train_loader):
                if self._gpu:
                    A, x0, embed, label = A.to(self._device), x0.to(self._device), embed.to(self._device), label.to(self._device)
                ...
                loss += lam * self._loss_func(output, label.unsqueeze(dim=1).float(), weight=torch.Tensor([self._loss_weights[i].item() for i in label]).unsqueeze(dim=1).to(device=self._device))\
                    if self._is_binary else lam * self._loss_func(output, label, weight=self._loss_weights.to(device=self._device))
                # calculate loss
                loss.backward()  # back propagation
                self._preconditioner.step(lam=lam)
        ...
        if self._nni:
            ...
            nni.report_final_result(min(dev_auc, train_auc))
        # -----------------------  FOR NNI  -------------------------/
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        #model = QGCN(params_file, ds.len_features, [10], d=0)
