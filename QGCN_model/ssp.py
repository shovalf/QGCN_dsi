...
class GCN(torch.nn.Module):
    def __init__(self, in_dim, out_dim, activation=torch.tanh):
    ...
    def forward(self, A, x0, act=True):
...
class CRD(torch.nn.Module):
    def __init__(self, in_dim, out_dim, activation, p):
        super(CRD, self).__init__()
        self.conv = GCN(in_dim, out_dim, activation)
        self.p = p
...
class CLS(torch.nn.Module):
    ...
    def forward(self, A, x):
        x = self.conv(A, x, act=False)
        x = F.log_softmax(x, dim=1)
        return x
        
        
class Net(torch.nn.Module):
    def __init__(self, num_features, hidden, num_classes, dropout, activation):
        super(Net, self).__init__()
        self.crd = CRD(num_features, hidden, activation, dropout)
